...
`/position-review`가 최초 테제를 조회할 때, `/post-mortem`이 청산가를 조회할 때 **가장 먼저** 참고하는 파일입니다. 실제로 보유 중이거나 보유했던 포지션만 기록합니다.
...
- **청산 시**: 행을 삭제하지 않는다. "상태" 칸을 `청산 (YYYY-MM-DD)`로 갱신하고, "청산가" 칸에 실제 청산가를 함께 기록해 판단 이력을 보존한다 (`/post-mortem`이 이 값을 조회한다).
...
- **`/post-mortem` 조회 대상**: "상태"가 `청산 (YYYY-MM-DD)`인 행만 대상이다. "청산가" 칸이 비어 있으면 실행 시 인자로 받거나 사용자에게 요청한다.
